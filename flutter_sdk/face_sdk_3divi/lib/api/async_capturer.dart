...
class AsyncCapturer {
  ...
  static Future<AsyncCapturer> create(Pointer<Void> implementation,
      String facerecConfDir, String dllPath, Config config) async {
  ...
  Future<List<RawSample>> capture(Uint8List encodedImage) async {
    ...
    for (var address in pointers) {
      result.add(RawSample(dylib, Pointer<Void>.fromAddress(address)));
    }
    ...
    await for (dynamic message in receivePort) {
      ...
      try {
        ...
        for (var sample in samples) {
          result.add(sample._impl.address);
        }
