...
class EyesOpennessResult {
  ...
  EyesOpennessResult(this.leftEyeVerdict, this.rightEyeVerdict,
      this.leftEyeScore, this.rightEyeScore);
}

class FaceAttributesEstimator extends _ComplexObject {
  FaceAttributesEstimator(super.dll_handle, super.impl);
  ...
  EyesOpennessResult estimateEyesOpenness(final RawSample sample) {
    final process = _dll_handle.lookupFunction<
            _FaceAttributesEstimator_estimateEyesOpenness,
            _FaceAttributesEstimator_estimateEyesOpenness>(
        '${_c_namespace}FaceAttributesEstimator_estimateEyesOpenness');

    Pointer<Int32> leftVerdictPtr =
        malloc.allocate<Int32>(sizeOf<Pointer<Int32>>());
    Pointer<Int32> rightVerdictPtr =
        malloc.allocate<Int32>(sizeOf<Pointer<Int32>>());
    Pointer<Float> leftScorePtr =
        malloc.allocate<Float>(sizeOf<Pointer<Float>>());
    Pointer<Float> rightScorePtr =
        malloc.allocate<Float>(sizeOf<Pointer<Float>>());

    final exception = _getException();
    process(_impl, sample._impl, leftVerdictPtr, rightVerdictPtr, leftScorePtr,
        rightScorePtr, exception);
    ...
    return EyesOpennessResult(leftVerdictValue == 1, rightVerdictValue == 1,
