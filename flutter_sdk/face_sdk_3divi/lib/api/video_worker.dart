...
class DataBaseElement {
  ...
  DataBaseElement(this.element_id, this.person_id, this.face_template,
...
class VideoWorkerParams {
  ...
  VideoWorkerParams short_time_identification_outdate_time_seconds(
      double value) {
  ...
  VideoWorkerParams active_liveness_checks_order(
      List<ActiveLivenessCheckType> value) {
...
class TrackingData {
  TrackingCallbackData tracking_callback_data = TrackingCallbackData();
  TemplateCreatedCallbackData template_created_callback_data =
      TemplateCreatedCallbackData();
  TrackingLostCallbackData tracking_lost_callback_data =
      TrackingLostCallbackData();
  TrackingMatchFoundCallbackData tracking_match_found_callback_data =
      TrackingMatchFoundCallbackData();
  StiPersonOutdatedCallbackData sti_person_outdated_callback_data =
...
class VideoWorker extends _ComplexObject {
  ...
  VideoWorker(DynamicLibrary dll_handle, Pointer<Void> impl)
    ...
    _getInt64 = dll_handle.lookupFunction<_StructStorage_get_int64_c,
            _StructStorage_get_int64_dart>(
        '${_c_namespace}StructStorage_get_int64');
    _getDouble = dll_handle.lookupFunction<_StructStorage_get_double_c,
            _StructStorage_get_double_dart>(
        '${_c_namespace}StructStorage_get_double');
    _getPointer = dll_handle.lookupFunction<_StructStorage_get_pointer_c,
            _StructStorage_get_pointer_dart>(
  ...
  TrackingCallbackData _parseTrackingCallback(
    ...
    final stream_id = getInt64(
        trackingCallbackPointer, StructStorageFields.video_worker_stream_id_t);

    final frame_id = getInt64(
        trackingCallbackPointer, StructStorageFields.video_worker_frame_id_t);

    final samples_count = getInt64(trackingCallbackPointer,
        StructStorageFields.video_worker_samples_count_t);

    Pointer<Pointer<Void>> samples = getPointer(
            trackingCallbackPointer, StructStorageFields.video_worker_samples_t)
        .cast();

    List<RawSample> rss = List<RawSample>.generate(
        samples_count, (i) => RawSample(_dll_handle, samples[i]));

    Pointer<Float> sample_quality = getPointer(trackingCallbackPointer,
    ...
    Pointer<Float> ALScore = getPointer(trackingCallbackPointer,
            StructStorageFields.video_worker_active_liveness_score_samples_t)
        .cast();
    Pointer<Int32> ALType = getPointer(trackingCallbackPointer,
            StructStorageFields.video_worker_active_liveness_type_samples_t)
        .cast();
    Pointer<Int32> ALConf = getPointer(
            trackingCallbackPointer,
            StructStorageFields
                .video_worker_active_liveness_confirmed_samples_t)
        .cast();
    ...
    for (int i = 0; i < samples_count; ++i) {
      ...
      if (status.progress_level > 1 ||
          ...
          (status.verdict == ActiveLiveness.ALL_CHECKS_PASSED &&
              status.progress_level != 1)) {
        if (_last_al.isNotEmpty) status = _last_al[i];
      }

      al.add(status);
    }

    TrackingCallbackData td = TrackingCallbackData();
  ...
  TemplateCreatedCallbackData _parseTemplateCreatedCallback(
      Pointer<Void> templateCallbackPointer) {
    final stream_id = getInt64(
        templateCallbackPointer, StructStorageFields.video_worker_stream_id_t);

    final frame_id = getInt64(
        templateCallbackPointer, StructStorageFields.video_worker_frame_id_t);

    final quality = getDouble(
        templateCallbackPointer, StructStorageFields.video_worker_quality_t);

    Pointer<Void> sample = getPointer(
        templateCallbackPointer, StructStorageFields.video_worker_samples_t);

    Pointer<Void> templ = getPointer(
        templateCallbackPointer, StructStorageFields.video_worker_templ_t);

    TemplateCreatedCallbackData tccd = TemplateCreatedCallbackData();
  ...
  TrackingLostCallbackData _parseTrackingLostCallback(
      Pointer<Void> trackingLostCallbackPointer) {
    final stream_id = getInt64(trackingLostCallbackPointer,
        StructStorageFields.video_worker_stream_id_t);

    final first_frame_id = getInt64(trackingLostCallbackPointer,
        StructStorageFields.video_worker_first_frame_id_t);

    final last_frame_id = getInt64(trackingLostCallbackPointer,
        StructStorageFields.video_worker_last_frame_id_t);

    final best_quality = getDouble(trackingLostCallbackPointer,
        StructStorageFields.video_worker_best_quality_t);

    final best_quality_frame_id = getInt64(trackingLostCallbackPointer,
        StructStorageFields.video_worker_best_qaulity_frame_id_t);

    Pointer<Void> best_quality_sample_impl = getPointer(
        trackingLostCallbackPointer,
        StructStorageFields.video_worker_samples_t);

    Pointer<Void> best_quality_template_impl = getPointer(
        trackingLostCallbackPointer, StructStorageFields.video_worker_templ_t);

    final track_id = getInt64(trackingLostCallbackPointer,
        StructStorageFields.video_worker_track_id_t);

    final sti_person_id_set = getInt64(trackingLostCallbackPointer,
        StructStorageFields.video_worker_sti_person_id_set_t);

    final sti_person_id = getInt64(trackingLostCallbackPointer,
        StructStorageFields.video_worker_sti_person_id_t);

    TrackingLostCallbackData data = TrackingLostCallbackData();
    ...
    if (best_quality_sample_impl != nullptr) {
      data.best_quality_sample =
          RawSample(_dll_handle, best_quality_sample_impl);
    }

    if (best_quality_template_impl != nullptr) {
      data.best_quality_templ =
          Template(_dll_handle, best_quality_template_impl);
    }
  ...
  TrackingMatchFoundCallbackData _parseTrackingMatchFoundCallback(
      Pointer<Void> trackingMatchFoundCallbackPointer) {
    final stream_id = getInt64(trackingMatchFoundCallbackPointer,
        StructStorageFields.video_worker_stream_id_t);
    final frame_id = getInt64(trackingMatchFoundCallbackPointer,
        StructStorageFields.video_worker_frame_id_t);
    final quality = getDouble(trackingMatchFoundCallbackPointer,
        StructStorageFields.video_worker_samples_quality_t);

    Pointer<Void> sample_impl = getPointer(trackingMatchFoundCallbackPointer,
        StructStorageFields.video_worker_samples_t);
    Pointer<Void> template_impl = getPointer(trackingMatchFoundCallbackPointer,
        StructStorageFields.video_worker_templ_t);

    final search_result_count = getInt64(trackingMatchFoundCallbackPointer,
        StructStorageFields.video_worker_search_result_size_t);
    Pointer<Int32> all_person_id = getPointer(trackingMatchFoundCallbackPointer,
            StructStorageFields.video_worker_search_result_person_ids_t)
        .cast();
    Pointer<Int32> all_element_id = getPointer(
            trackingMatchFoundCallbackPointer,
            StructStorageFields.video_worker_search_result_element_ids_t)
        .cast();
    Pointer<Float> all_distance = getPointer(
            trackingMatchFoundCallbackPointer,
            StructStorageFields
                .video_worker_search_result_match_result_distance_t)
        .cast();
    Pointer<Float> all_far = getPointer(trackingMatchFoundCallbackPointer,
            StructStorageFields.video_worker_search_result_match_result_fa_r_t)
        .cast();
    Pointer<Float> all_frr = getPointer(trackingMatchFoundCallbackPointer,
            StructStorageFields.video_worker_search_result_match_result_fr_r_t)
        .cast();
    Pointer<Float> all_score = getPointer(trackingMatchFoundCallbackPointer,
    ...
    TrackingMatchFoundCallbackData data = TrackingMatchFoundCallbackData();
    data.stream_id = stream_id;
  ...
  StiPersonOutdatedCallbackData _parseStiPersonOutdatedCallbackData(
      Pointer<Void> stiPersonOutdatedCallbackPointer) {
    final stream_id = getInt64(stiPersonOutdatedCallbackPointer,
        StructStorageFields.video_worker_stream_id_t);
    final sti_person_id = getInt64(stiPersonOutdatedCallbackPointer,
        StructStorageFields.video_worker_sti_person_id_t);

    StiPersonOutdatedCallbackData data = StiPersonOutdatedCallbackData();
    data.stream_id = stream_id;
    data.sti_person_id = sti_person_id;
  ...
  TrackingData poolTrackResults() {
    TrackingData trackData = TrackingData();
    ...
    final track_callback = getPointer(
        storage, StructStorageFields.video_worker_tracking_callback_t);
    ...
    final templ_create_callback = getPointer(
        storage, StructStorageFields.video_worker_template_created_callback_t);

    if (templ_create_callback.address != nullptr.address) {
      trackData.template_created_callback_data =
          _parseTemplateCreatedCallback(templ_create_callback);
    }

    final lost_callback = getPointer(
        storage, StructStorageFields.video_worker_tracking_lost_callback_t);

    if (lost_callback.address != nullptr.address) {
      trackData.tracking_lost_callback_data =
          _parseTrackingLostCallback(lost_callback);
    }

    final match_found_callback = getPointer(
        storage, StructStorageFields.video_worker_match_found_callback_t);
    if (match_found_callback.address != nullptr.address) {
      trackData.tracking_match_found_callback_data =
          _parseTrackingMatchFoundCallback(match_found_callback);
    }

    final sti_person_callback = getPointer(
        storage, StructStorageFields.video_worker_tracking_sti_callback_t);
    if (sti_person_callback.address != nullptr.address) {
      trackData.sti_person_outdated_callback_data =
          _parseStiPersonOutdatedCallbackData(sti_person_callback);
    }
  ...
  int addVideoFrame(RawImageF image, int timestamp_microsec) {
    final addVF =
        _dll_handle.lookupFunction<_VWaddVideoFrame_c, _VWaddVideoFrame_dart>(
  ...
  void resetTrackerOnStream() {
    final reset = _dll_handle.lookupFunction<_VWresetTrackerOnStream_c,
            _VWresetTrackerOnStream_dart>(
  ...
  void setDataBase(List<DataBaseElement> dataBase,
      {SearchAccelerationType acceleration =
          SearchAccelerationType.SEARCH_ACCELERATION_1}) {
    final set_db =
        _dll_handle.lookupFunction<_VWsetDataBase, _VWsetDataBase_dart>(
            '${_c_namespace}VideoWorker_setDatabase');

    final data_base_size = dataBase.length;
    Pointer<Uint64> element_ids_v =
        malloc.allocate(data_base_size * sizeOf<Uint64>());
    Pointer<Uint64> person_ids_v =
        malloc.allocate(data_base_size * sizeOf<Uint64>());
    Pointer<Float> thresholds_v =
        malloc.allocate(data_base_size * sizeOf<Float>());
    Pointer<Pointer<Void>> templates_v =
        malloc.allocate(data_base_size * sizeOf<Pointer<Pointer<Void>>>());
    ...
    Pointer<Pointer<Void>> exception = _getException();
    set_db(_impl, acceleration.index, data_base_size, element_ids_v,
        person_ids_v, templates_v, thresholds_v, exception);
    checkException(exception, _dll_handle);
