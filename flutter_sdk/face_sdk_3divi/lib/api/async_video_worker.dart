...
enum _VideoWorkerEvents {
  ADD_VIDEO_FRAME,
  SET_DATA_BASE,
  RESET_TRACKER_ON_STREAM,
  POOL_TRACK_RESULTS,
  CLEAR
}
...
class AsyncVideoWorker {
  ...
  static Future<AsyncVideoWorker> create(Pointer<Void> implementation,
      String facerecConfDir, String dllPath, VideoWorkerParams params) async {
    ...
    Isolate isolate = await Isolate.spawn(_isolateImplementation, {
      ...
      "_age_gender_estimation_threads_count":
          params._age_gender_estimation_threads_count,
      "_emotions_estimation_threads_count":
          params._emotions_estimation_threads_count,
      "_short_time_identification_enabled":
          params._short_time_identification_enabled,
      "_short_time_identification_distance_threshold":
          params._short_time_identification_distance_threshold,
      "_short_time_identification_outdate_time_seconds":
  ...
  Future<void> setDataBase(List<DataBaseElement> dataBase,
      {SearchAccelerationType acceleration =
          SearchAccelerationType.SEARCH_ACCELERATION_1}) async {
    ReceivePort receivePort = ReceivePort();
    List<Map<String, dynamic>> data = [];

    for (var element in dataBase) {
      Map<String, dynamic> elementData = <String, dynamic>{};
  ...
  Future<TrackingData> poolTrackResults() async {
    ...
    result.tracking_callback_data =
        _parseTrackingCallbackData(data["tracking_callback_data"]);
    result.template_created_callback_data = _parseTemplateCreatedCallbackData(
        data["template_created_callback_data"]);
    result.tracking_lost_callback_data =
        _parseTrackingLostCallbackData(data["tracking_lost_callback_data"]);
    result.tracking_match_found_callback_data =
        _parseTrackingMatchFoundCallbackData(
            data["tracking_match_found_callback_data"]);
    result.sti_person_outdated_callback_data =
        _parseStiPersonOutdatedCallbackData(
            data["sti_person_outdated_callback_data"]);
  ...
  static Future<void> _isolateImplementation(
      Map<String, dynamic> initialization) async {
    ...
    overriddenParams
        .forEach((key, value) => config.overrideParameter(key, value));
    ...
    params._processing_threads_count =
        initialization["_processing_threads_count"];
    params._matching_threads_count = initialization["_matching_threads_count"];
    params._age_gender_estimation_threads_count =
        initialization["_age_gender_estimation_threads_count"];
    params._emotions_estimation_threads_count =
        initialization["_emotions_estimation_threads_count"];
    params._short_time_identification_enabled =
        initialization["_short_time_identification_enabled"];
    params._short_time_identification_distance_threshold =
    ...
    params._active_liveness_checks_order =
        initialization["_active_liveness_checks_order"];
        ...
        switch (event) {
          case _VideoWorkerEvents.ADD_VIDEO_FRAME:
            ...
            int crop_info_data_image_width =
                message["crop_info_data_image_width"];
            int crop_info_data_image_height =
                message["crop_info_data_image_height"];
          ...
          case _VideoWorkerEvents.SET_DATA_BASE:
            ...
            for (var element in data) {
              dataBase.add(DataBaseElement(
                  ...
                  element["person_id"],
                  Template(dylib,
                      Pointer<Void>.fromAddress(element["face_template"])),
          ...
          case _VideoWorkerEvents.POOL_TRACK_RESULTS:
            ...
            for (var element in data.tracking_callback_data.samples) {
              samplePointers.add(element._impl.address);
            }
            List<ActiveLivenessStatus> status =
                data.tracking_callback_data.samples_active_liveness_status;
            ...
            if (data.tracking_lost_callback_data.stream_id != -1) {
              bestQualitySample =
                  data.tracking_lost_callback_data.best_quality_sample;
              bestQualityTemplate =
                  data.tracking_lost_callback_data.best_quality_templ;
            }

            if (data.tracking_match_found_callback_data.stream_id != -1) {
              matchFoundSample = data.tracking_match_found_callback_data.sample;
              matchFoundTemplate =
                  data.tracking_match_found_callback_data.templ;
            }
            ...
            result["tracking_callback_data"] = {
              ...
              "_samples_track_id":
                  data.tracking_callback_data._samples_track_id,
            ...
            result["tracking_lost_callback_data"] = {
              ...
              "best_quality_frame_id":
                  data.tracking_lost_callback_data.best_quality_frame_id,
              "track_id": data.tracking_lost_callback_data.track_id,
              "sti_person_id_set":
                  data.tracking_lost_callback_data.sti_person_id_set,
              "sti_person_id": data.tracking_lost_callback_data.sti_person_id
            };

            if (bestQualitySample != null) {
              result["tracking_lost_callback_data"]["best_quality_sample"] =
                  bestQualitySample._impl.address;
            }

            if (bestQualityTemplate != null) {
              result["tracking_lost_callback_data"]["best_quality_template"] =
                  bestQualityTemplate._impl.address;
            }

            List<Map<String, dynamic>> searchResults = [];

            for (VWSearchResult searchResult
                in data.tracking_match_found_callback_data.search_results) {
            ...
            if (matchFoundSample != null) {
              result["tracking_match_found_callback_data"]["sample"] =
                  matchFoundSample._impl.address;
            }

            if (matchFoundTemplate != null) {
              result["tracking_match_found_callback_data"]["template"] =
                  matchFoundTemplate._impl.address;
            }

            result["sti_person_outdated_callback_data"] = {
              "stream_id": data.sti_person_outdated_callback_data.stream_id,
              "sti_person_id":
                  data.sti_person_outdated_callback_data.sti_person_id
            };
  ...
  TrackingCallbackData _parseTrackingCallbackData(Map<String, dynamic> data) {
    ...
    List<ActiveLiveness> verdict =
        data["samples_active_liveness_status"]["verdict"];
    List<ActiveLivenessCheckType> check_type =
        data["samples_active_liveness_status"]["check_type"];
    List<double> progress_level =
        data["samples_active_liveness_status"]["progress_level"];

    for (var pointer in pointers) {
      result.samples.add(RawSample(dylib, Pointer<Void>.fromAddress(pointer)));
    }
  ...
  TemplateCreatedCallbackData _parseTemplateCreatedCallbackData(
    ...
    if (result.stream_id != -1) {
      result.sample =
          RawSample(dylib, Pointer<Void>.fromAddress(data["sample"]));
      result.templ =
          Template(dylib, Pointer<Void>.fromAddress(data["template"]));
    }

    return result;
  }

  TrackingLostCallbackData _parseTrackingLostCallbackData(
      Map<String, dynamic> data) {
    DynamicLibrary dylib = DynamicLibrary.open(_dllPath);
    ...
    if (data.containsKey("best_quality_sample")) {
      result.best_quality_sample = RawSample(
          dylib, Pointer<Void>.fromAddress(data["best_quality_sample"]));
    }

    if (data.containsKey("best_quality_template")) {
      result.best_quality_templ = Template(
          dylib, Pointer<Void>.fromAddress(data["best_quality_template"]));
    }

    return result;
  }

  TrackingMatchFoundCallbackData _parseTrackingMatchFoundCallbackData(
      Map<String, dynamic> data) {
    DynamicLibrary dylib = DynamicLibrary.open(_dllPath);
    ...
    for (var element in results) {
      ...
      result.search_results
          .add(VWSearchResult(person_id, element_id, matchResult));
    }
