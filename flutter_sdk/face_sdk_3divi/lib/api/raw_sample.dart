...
class PointerBuffer<T extends NativeType> {
  ...
  PointerBuffer(
      this.ptr, this._elementSizePointer, this._sizePointer, this._dataPointer);
  ...
  PointerBuffer.fromSize(int elementSize, int length) {
    ...
    if (_dataPointer.value.address == nullptr.address) {
      throw Exception(
          "RawSample.fromSize: Failed to allocate a ${elementSize * length} bytes");
    }
  ...
  int get elementSize {
    return _elementSizePointer.address == nullptr.address
        ? -1
        : _elementSizePointer.value;
  }
  ...
  void setSize(int newLength) {
    ...
    if (_dataPointer.value.address == nullptr.address) {
      throw Exception(
          "RawSample.setSize: Failed to allocate a ${elementSize * newLength} bytes");
    }
  }
...
class WrapBytesWrite {
  static void writeByteListFunc(
      Pointer<Void> stream, Pointer<Uint8> data, int bytesCount) {
  ...
  static void writeFloatListFunc(Pointer<Void> pointersVector,
      Pointer<Float> elements, int elementsCount) {
    PointerBuffer buffer = PointerBuffer<Float>.fromPointer(pointersVector);
    buffer.setSize(elementsCount);

    Float32List streamDataPtr =
        buffer.dataPtr<Float>().asTypedList(elementsCount);
...
class RawSample extends _ComplexObject {
  RawSample(super.dll_handle, super.impl);

  /// Get a face bounding rectangle.
  Rectangle getRectangle() {
    var exception = _getException();
    final get_rect =
        _dll_handle.lookupFunction<_RSgetRectangle_c, _RSgetRectangle_dart>(
  ...
  Point _getEye(String sideEye) {
    assert(sideEye == "Right" || sideEye == "Left",
        "Available values: Right or Left");
  ...
  double getScore() {
    ...
    final double res = get(_impl, exception);
    checkException(exception, _dll_handle);
  ...
  List<double> getLandmarks() {
    ...
    final get =
        _dll_handle.lookupFunction<_RSgetLandmarks_c, _RSgetLandmarks_dart>(
            '${_c_namespace}RawSample_getLandmarks');

    PointerBuffer<Float> buffer = PointerBuffer<Float>.empty(sizeOf<Float>());

    get(_impl, buffer.ptr.cast(),
        Pointer.fromFunction(WrapBytesWrite.writeFloatListFunc), exception);
  ...
  Uint8List cutFaceImage(ImageFormatCode imageFormat, FaceCutType cutType) {
    ...
    if (widthPtr.address == nullptr.address ||
        heightPtr.address == nullptr.address) {
      throw Exception(
          "RawSample.cutFaceImage: Failed to allocate a ${sizeOf<Pointer<Int32>>()} bytes");
    }

    final get =
        _dll_handle.lookupFunction<_RScutFaceImage_c, _RScutFaceImage_dart>(
  ...
  RawImageF cutFaceRawImage(Format colorFormat, FaceCutType cutType) {
    ...
    Pointer<Int32> widthPtr = malloc.allocate(sizeOf<Pointer<Int32>>());
    Pointer<Int32> heightPtr = malloc.allocate(sizeOf<Pointer<Int32>>());
    Pointer<Pointer<Void>> bufer =
        malloc.allocate(sizeOf<Pointer<Pointer<Uint8>>>());
  ...
  Context toContext() {
    ...
    final ctxPointer = func(_impl, exception);
