...
class AsyncRecognizer {
  ...
  Future<Template> processing(RawSample sample) async {
    ...
    _sendPort.send({
      "event": _RecognizerEvents.PROCESSING,
      "sendPort": receivePort.sendPort,
      "rawSample": sample._impl.address
    });

    result = await receivePort.first;

    return Template(
        DynamicLibrary.open(_dllPath), Pointer<Void>.fromAddress(result));
  }

  Future<MatchResult> verifyMatch(
      Template template1, Template template2) async {
    ReceivePort receivePort = ReceivePort();
    ...
    return MatchResult(result["distance"]!, result["fa_r"]!, result["fr_r"]!,
        result["score"]!);
  }
  ...
  Future<Template> loadTemplate(Uint8List binary_stream) async {
    ...
    _sendPort.send({
      "event": _RecognizerEvents.LOAD_TEMPLATE,
      "sendPort": receivePort.sendPort,
  ...
  Future<TemplateIndex> createIndex(List<Template> templates,
      int search_threads_count, int reserve_queries_count) async {
    ReceivePort receivePort = ReceivePort();

    List<int> pointers = [];

    for (var template in templates) {
      pointers.add(template._impl.address);
    }
  ...
  Future<List<SearchResult>> singleSearch(
    ...
    for (Map<String, dynamic> mapData in data) {
      result.add(SearchResult(
          mapData["i"],
          MatchResult(mapData["distance"]!, mapData["fa_r"]!, mapData["fr_r"]!,
              mapData["score"]!)));
    }
  ...
  Future<List<List<SearchResult>>> search(
    ...
    for (var template in queries_templates) {
      pointers.add(template._impl.address);
    }
    ...
    for (List<Map<String, dynamic>> listMapData in data) {
      ...
      for (Map<String, dynamic> mapData in listMapData) {
        searchResults.add(SearchResult(
            mapData["i"],
            MatchResult(mapData["distance"]!, mapData["fa_r"]!,
                mapData["fr_r"]!, mapData["score"]!)));
      }
  ...
  static Future<AsyncRecognizer> create(
      Pointer<Void> implementation,
      String facerecConfDir,
      String dllPath,
      String ini_file,
      bool processing,
      bool matching,
  ...
  static Future<void> _isolateImplementation(
    ...
    bool processing_less_memory_consumption =
        initialization["processing_less_memory_consumption"];
      ...
      try {
        switch (event) {
          case _RecognizerEvents.PROCESSING:
            Template result = recognizer.processing(RawSample(
                dylib, Pointer<Void>.fromAddress(message["rawSample"])));
          ...
          case _RecognizerEvents.VERIFY_MATCH:
            Template template1 = Template(
                dylib, Pointer<Void>.fromAddress(message["template1"]));
            Template template2 = Template(
                dylib, Pointer<Void>.fromAddress(message["template2"]));
          ...
          case _RecognizerEvents.CREATE_INDEX:
            ...
            for (var pointer in pointers) {
              templates
                  .add(Template(dylib, Pointer<Void>.fromAddress(pointer)));
            }

            TemplateIndex templateIndex = recognizer.createIndex(
                templates, search_threads_count, reserve_queries_count);
          ...
          case _RecognizerEvents.SINGLE_SEARCH:
            Template template =
                Template(dylib, Pointer<Void>.fromAddress(message["template"]));
            TemplateIndex templateIndex = TemplateIndex(
                dylib, Pointer<Void>.fromAddress(message["templateIndex"]));
            int k = message["k"];
            SearchAccelerationType acceleration = message["acceleration"];
            List<Map<String, dynamic>> result = [];

            List<SearchResult> data = recognizer.singleSearch(
                template, templateIndex, k,
                acceleration: acceleration);
          ...
          case _RecognizerEvents.SEARCH:
            ...
            List<List<SearchResult>> data = recognizer.search(
                queries_templates, templateIndex, k,
                acceleration: acceleration);
