...
class RawImageF {
  ...
  static RawImageF emptyBuffer(int width, int height,
      {Format format = Format.FORMAT_RGB}) {
    if (!_formatMap.containsKey(format)) {
      throw Exception("Unsuport image format");
    }
    ...
    if (dataPtr.address == nullptr.address) {
      throw Exception(
          "RawImageF.emptyBuffer: Failed to allocate a $size bytes");
    }

    return RawImageF(width, height, format, dataPtr.cast());
  }

  static RawImageF fromImage(imglib.Image image,
      {Format format = Format.FORMAT_RGB}) {
    if (!_formatMap.containsKey(format)) {
      throw Exception("Unsuport image format");
    }
    ...
    if (dataPtr.address == nullptr.address) {
      throw Exception(
          "RawImageF.fromImage: Failed to allocate a ${data.length} bytes");
    }
  ...
  static final Map<Format, imglib.ChannelOrder> _formatMap = {
  ...
  imglib.Image? getImage() {
    ...
    return imglib.Image.fromBytes(
        width: width,
        height: height,
        bytes: rawFrameData.buffer,
        order: _formatMap[format]!);
  }
}
