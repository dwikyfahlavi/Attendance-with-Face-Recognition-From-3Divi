...
void readFunc(Pointer<Void> stream, Pointer<Void> data, int bytesCount) {
  ...
  Pointer<Int32> currentByteCount =
      Pointer.fromAddress(buffer_stream[0].address);
...
class Recognizer extends _ComplexObject {
  Recognizer(super.dll_handle, super.impl);

  /// Create a template from detected face [sample] (from Capturer of VideoWorker).
  Template processing(final RawSample sample) {
    final process = _dll_handle.lookupFunction<_RecognizerProcessing,
        _RecognizerProcessing>('${_c_namespace}Recognizer_processing');
  ...
  MatchResult verifyMatch(Template template1, Template template2) {
    ...
    final get = _dll_handle.lookupFunction<_Recognizer_verifyMatch_v2_c,
            _Recognizer_verifyMatch_v2_dart>(
    ...
    get(_impl, template1._impl, template2._impl, distance, far, frr, score,
        exception);
    checkException(exception, _dll_handle);

    MatchResult result =
        MatchResult(distance.value, far.value, frr.value, score.value);
  ...
  Template loadTemplate(Uint8List binary_stream) {
    var exception = _getException();
    final loadTemplate = _dll_handle.lookupFunction<_Recognizer_load_template_c,
            _Recognizer_load_template_dart>(
        '${_c_namespace}Recognizer_loadTemplate');

    Pointer<Pointer<Uint8>> _templatePointerStructure =
    ...
    Pointer<Int32> byteCount =
        Pointer.fromAddress(_templatePointerStructure[0].address);
    byteCount.value = 0;
    ...
    final templPointer = loadTemplate(_impl, _templatePointerStructure.cast(),
  ...
  TemplateIndex createIndex(List<Template> templates, int search_threads_count,
      int reserve_queries_count) {
    var exception = _getException();
    final _createIndex = _dll_handle.lookupFunction<_Recognizer_create_index_c,
        _Recognizer_create_index_dart>('${_c_namespace}Recognizer_createIndex');

    Pointer<Pointer<Void>> templatesImpl =
        malloc.allocate(sizeOf<Pointer<Pointer<Void>>>() * templates.length);
    for (var i = 0; i < templates.length; i++) {
      templatesImpl[i] = templates[i]._impl;
    }

    final templIndexPointer = _createIndex(_impl, templates.length,
        templatesImpl, search_threads_count, exception);
    checkException(exception, _dll_handle);
  ...
  List<SearchResult> singleSearch(
      Template template, TemplateIndex templatesIndex, int k,
      {SearchAccelerationType acceleration =
          SearchAccelerationType.SEARCH_ACCELERATION_1}) {
    final _search = _dll_handle.lookupFunction<_Recognizer_search_v2_c,
        _Recognizer_search_v2_dart>('${_c_namespace}Recognizer_search_v2');
    final exception = _getException();

    Pointer<Pointer<Void>> templatesImpl =
        malloc.allocate(sizeOf<Pointer<Pointer<Void>>>());
    ...
    for (var i = 0; i < k; i++) {
      result.add(SearchResult(
          result_i[i],
          MatchResult(result_distance[i], result_far[i], result_frr[i],
              result_score[i])));
    }
  ...
  List<List<SearchResult>> search(
      List<Template> queries_templates, TemplateIndex templates_index, int k,
      {SearchAccelerationType acceleration =
          SearchAccelerationType.SEARCH_ACCELERATION_1}) {
    final _search = _dll_handle.lookupFunction<_Recognizer_search_v2_c,
        _Recognizer_search_v2_dart>('${_c_namespace}Recognizer_search_v2');
    ...
    Pointer<Pointer<Void>> queriesTemplatesImpl = malloc
        .allocate(sizeOf<Pointer<Pointer<Void>>>() * queries_templates.length);
    for (var i = 0; i < queries_templates.length; i++) {
      queriesTemplatesImpl[i] = queries_templates[i]._impl;
    }

    Pointer<Int64> result_i =
        malloc.allocate(k * queries_count * sizeOf<Int64>());
    Pointer<Float> result_distance =
        malloc.allocate(k * queries_count * sizeOf<Float>());
    Pointer<Float> result_far =
        malloc.allocate(k * queries_count * sizeOf<Float>());
    Pointer<Float> result_frr =
        malloc.allocate(k * queries_count * sizeOf<Float>());
    Pointer<Float> result_score =
        malloc.allocate(k * queries_count * sizeOf<Float>());
  ...
  MatchResult getRocCurvePointByFar(double far_threshold) {
    final get_roc_curve = _dll_handle.lookupFunction<
            _Recognizer_get_roc_curve_c, _Recognizer_get_roc_curve_dart>(
        '${_c_namespace}Recognizer_getROCCurvePointByFAR_v2');
    ...
    final exception = _getException();
    get_roc_curve(_impl, far_threshold, result_distance, result_far, result_frr,
        result_score, exception);
    checkException(exception, _dll_handle);

    final result = MatchResult(result_distance.value, result_far.value,
        result_frr.value, result_score.value);
  ...
  MatchResult getRocCurvePointByFrr(double frr_threshold) {
    final get_roc_curve = _dll_handle.lookupFunction<
            _Recognizer_get_roc_curve_c, _Recognizer_get_roc_curve_dart>(
        '${_c_namespace}Recognizer_getROCCurvePointByFRR_v2');
    ...
    final exception = _getException();
    get_roc_curve(_impl, frr_threshold, result_distance, result_far, result_frr,
        result_score, exception);
    checkException(exception, _dll_handle);
  ...
  MatchResult getRocCurvePointByScore(double score_threshold) {
    final get_roc_curve = _dll_handle.lookupFunction<
            _Recognizer_get_roc_curve_c, _Recognizer_get_roc_curve_dart>(
        '${_c_namespace}Recognizer_getROCCurvePointByScoreThreshold');
    ...
    final exception = _getException();
    get_roc_curve(_impl, score_threshold, result_distance, result_far,
        result_frr, result_score, exception);
    checkException(exception, _dll_handle);
  ...
  MatchResult getRocCurvePointByDistance(double distance_threshold) {
    final get_roc_curve = _dll_handle.lookupFunction<
            _Recognizer_get_roc_curve_c, _Recognizer_get_roc_curve_dart>(
        '${_c_namespace}Recognizer_getROCCurvePointByDistanceThreshold_v2');
    ...
    final exception = _getException();
    get_roc_curve(_impl, distance_threshold, result_distance, result_far,
        result_frr, result_score, exception);
    checkException(exception, _dll_handle);
