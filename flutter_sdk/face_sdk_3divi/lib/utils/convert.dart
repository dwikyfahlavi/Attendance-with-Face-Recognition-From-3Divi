...
class NativeDataStruct {
  ...
  bool get isValid {
    return pointer != null &&
        bytes != null &&
        pointer!.address != nullptr.address;
  }
  ...
  void resize(int byteCount) {
    ...
    if (pointer!.address == nullptr.address) {
      throw Exception(
          "NativeDataStruct.resize: Failed to allocate a $byteCount bytes");
    }
...
Future<imglib.Image> cutFaceImageFromImageBytes(
    Uint8List imgBytes, Rectangle rect) async {
  ...
  final img2 = imglib.copyCrop(bufImage,
      x: rect.x, y: rect.y, width: rect.width, height: rect.height);
